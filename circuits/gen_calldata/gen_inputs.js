...
const provingKeyGen = "./circuits/gen_calldata/setup.sh";
const proofGen = "./circuits/gen_calldata/proof_gen.sh";
/**
...
const generate_proofs = async() => {
    const proofPromises = [];
    for(let i=0; i<3; ++i){
        let promise = new Promise((resolve, _) => {
                exec(`bash ${proofGen} ${i+1}`, (error, stdout, stderr) => {
                    if (error) {
                        console.error(`Error: ${error.message}`);
                        console.error(`Exit code: ${error.code}`);
                        if (stderr) console.error(`stderr:\n${stderr}`);
...
(async() => {
    await runBashScript(provingKeyGen);
